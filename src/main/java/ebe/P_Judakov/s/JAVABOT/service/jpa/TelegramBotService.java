    ...
    private void sendStockQuoteInfo(long chatId, String stockTicker, ReplyKeyboardMarkup keyboardMarkup) {
        try {
            ...
            if (keyboardMarkup != null) {
                ...
                System.out.println("Отправка информации о котировках акции прошла успешно");
            } else {
                sendTextMessage(chatId, "Информации о котировках акции " + stockTicker + " :");
                System.out.println("Отправка информации о котировках акции прошла успешно");
            }
        } catch (Exception e) {
            System.err.println("Ошибка при запросе к API или отправке сообщения");
            LOGGER.error("Ошибка при запросе к API или отправке сообщения", e);
            e.printStackTrace();
        }
    }
...
//    }
     ...
     */
    @Override
    public org.telegram.telegrambots.meta.api.objects.Message execute(SendMessage message) {
        try {
    ...
    public static class StockQuoteBot extends TelegramLongPollingBot implements IStockQuoteBot {
        ...
        public void sendStockQuoteInfo(long chatId, String stockQuoteInfo, ReplyKeyboardMarkup keyboardMarkup) throws TelegramApiException {
            ...
            // Вызов метода для сохранения сообщения в базе данных
            TelegramMessageHandler messageHandler = new TelegramMessageHandler();
            messageHandler.handleMessage(chatId, message);

             // Логирование сохранения сообщения в базе данных
            logger.info("Сообщение успешно сохранено в базе данных для чата с ID: {}", chatId);
            System.out.println("Сообщение успешно сохранено в базе данных для чата с ID");
        }
